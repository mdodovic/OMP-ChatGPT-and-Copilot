...
# pragma omp parallel for private(i) shared(n3, x, vh, f, side) schedule(static)
    for (i=0; i<n3; i++) {
      x[i] += vh[i]+f[i];
  /*
...
# pragma omp parallel for private(i, j, xi, yi, zi, fxi, fyi, fzi, xx, yy, zz, rd, rrd, rrd2, rrd3, rrd4, rrd6, rrd7, r148, forcex, forcey, forcez) shared(npart, x, f, side, sideh, rcoffs) reduction(+ \
