...
#pragma omp parallel for default(none) shared(curr, n) private(speed_sqr) reduction(+:ke)
   for (i = 0; i < n; i++) {
...
#pragma omp parallel for default(none) shared(curr, n) private(diff, dist) reduction(+:pe) collapse(2)
   for (i = 0; i < n-1; i++) {
      for (j = i+1; j < n; j++) {
