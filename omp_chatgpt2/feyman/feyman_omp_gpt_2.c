...
int main(int argc, char **argv)
{
    ...
    #pragma omp parallel for reduction(+:err,n_inside) private(i,j,k,x,y,z,chk,w_exact,wt,steps,trial,x1,x2,x3,w,vh,we,vs,dx,dy,dz,ut,us) schedule(dynamic)
    for (i = 1; i <= ni; i++)
    {
        int local_seed = seed + omp_get_thread_num(); // Create a thread-local seed
        // using local_seed instead of seed for the r8_uniform_01 calls
