 ...
 */
  ...
  domove(int n3, double x[], double vh[], double f[], double side){
    ...
    #pragma omp parallel for default(none) shared(x, vh, f, side, n3) schedule(static)
    for (int i = 0; i < n3; i++) {
        x[i] += vh[i] + f[i];
        if (x[i] < 0.0)  x[i] += side;
...
#pragma omp parallel for default(none) shared(npart, sideh, side, rcoffs, x, f) private(xi, yi, zi, fxi, fyi, fzi, xx, yy, zz, rd, rrd, rrd2, rrd3, rrd4, rrd6, rrd7, r148, forcex, forcey, forcez) \
                                                                                                                                                                 reduction(+:epot, vir) schedule(dynamic)
for (int i = 0; i < npart * 3; i += 3)
{
    ...
    {
        //double xx, yy, zz, rd, rrd, rrd2, rrd3, rrd4, rrd6, rrd7, r148, forcex, forcey, forcez;
        xx = xi - x[j];
        yy = yi - x[j + 1];
        zz = zi - x[j + 2];
        if (xx < -sideh)
